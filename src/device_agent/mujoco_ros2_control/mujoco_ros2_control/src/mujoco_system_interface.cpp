                                    ...
                                    const hardware_interface::HardwareInfo& hardware_info, const mjModel* mj_model)
{
  ...
  const auto hw_joint_it = std::find_if(
      hardware_info.joints.begin(), hardware_info.joints.end(),
      [&joint_name](const hardware_interface::ComponentInfo& info) { return info.name == joint_name; });
  if (hw_joint_it != hardware_info.joints.end())
  {
    const auto param_it = hw_joint_it->parameters.find("mujoco_actuator");
    if (param_it != hw_joint_it->parameters.end() && !param_it->second.empty())
    {
      RCLCPP_DEBUG(rclcpp::get_logger("MujocoSystemInterface"),
                   "Mapped joint '%s' to actuator '%s' using ros2_control parameter 'mujoco_actuator'",
                   joint_name.c_str(), param_it->second.c_str());
  ...
  for (const auto& transmission : hardware_info.transmissions)
  {
    for (const auto& joint : transmission.joints)
    {
      if (joint.name == joint_name)
      {
        ...
        if (transmission.actuators.size() == 1)
        {
          RCLCPP_DEBUG(rclcpp::get_logger("MujocoSystemInterface"),
                       "Mapped joint '%s' to actuator '%s' using single-actuator transmission '%s'",
                       joint_name.c_str(), transmission.actuators.front().name.c_str(), transmission.name.c_str());
          return transmission.actuators.front().name;
        }
...
double get_joint_actuator_scale(const std::string& joint_name, const hardware_interface::HardwareInfo& hardware_info)
{
  const auto hw_joint_it = std::find_if(
      hardware_info.joints.begin(), hardware_info.joints.end(),
      [&joint_name](const hardware_interface::ComponentInfo& info) { return info.name == joint_name; });
  if (hw_joint_it != hardware_info.joints.end())
  {
    const auto param_it = hw_joint_it->parameters.find("mujoco_actuator_scale");
    if (param_it != hw_joint_it->parameters.end() && !param_it->second.empty())
    {
...
#endif
{
  ...
  RCLCPP_INFO_EXPRESSION(get_logger(), disable_rendering_, "Camera and lidar rendering disabled.");
...
hardware_interface::return_type MujocoSystemInterface::read(const rclcpp::Time& time, const rclcpp::Duration& /*period*/)
{
  ...
  {
    if (actuator_state.is_actuator_only)
    {
      actuator_state.position_interface.state_ = mj_data_control_->ctrl[actuator_state.mj_actuator_id];
      actuator_state.velocity_interface.state_ = 0.0;
      actuator_state.effort_interface.state_ = mj_data_control_->actuator_force[actuator_state.mj_actuator_id];
    }
    else
    {
...
void MujocoSystemInterface::actuator_state_to_joint_state()
{
  ...
  for (auto& joint : urdf_joint_data_)
  {
    const auto actuator_name = get_joint_actuator_name(joint.name, get_hardware_info(), mj_model_);
    const double scale = get_joint_actuator_scale(joint.name, get_hardware_info());
    bool copied_from_same_named_joint = false;

    // Prefer the state from a same-named MuJoCo joint when it exists. This keeps the ROS-side
    // joint state aligned with the actual simulated qpos/qvel, even if commands are mapped through
    // a different MuJoCo actuator (for example a tendon actuator driving a gripper).
    std::for_each(mujoco_actuator_data_.begin(), mujoco_actuator_data_.end(), [&](auto& actuator_interface) {
      if (actuator_interface.joint_name == joint.name)
      {
        ...
        copied_from_same_named_joint = true;
      }
    });

    // Fall back to the mapped actuator state only when there is no same-named MuJoCo joint state.
    if (!copied_from_same_named_joint)
    {
      std::for_each(mujoco_actuator_data_.begin(), mujoco_actuator_data_.end(), [&](auto& actuator_interface) {
        if (actuator_interface.joint_name == actuator_name)
        {
          joint.position_interface.state_ = actuator_interface.position_interface.state_ / scale;
          joint.velocity_interface.state_ = actuator_interface.velocity_interface.state_ / scale;
          joint.effort_interface.state_ = actuator_interface.effort_interface.state_;
        }
      });
    }
  ...
  // Explicitly propagate mimic joint states so visualizers such as RViz receive
  // the mirrored finger state even when only the driving joint is controlled.
  for (auto& joint : urdf_joint_data_)
  {
    if (joint.is_mimic)
    {
      const auto& source_joint = urdf_joint_data_.at(joint.mimicked_joint_index);
      joint.position_interface.state_ = joint.mimic_multiplier * source_joint.position_interface.state_;
      joint.velocity_interface.state_ = joint.mimic_multiplier * source_joint.velocity_interface.state_;
      joint.effort_interface.state_ = joint.mimic_multiplier * source_joint.effort_interface.state_;
    }
  }
}
...
void MujocoSystemInterface::joint_command_to_actuator_command()
{
  ...
  for (auto& joint : urdf_joint_data_)
  {
    ...
    const double scale = get_joint_actuator_scale(joint.name, get_hardware_info());
    std::for_each(mujoco_actuator_data_.begin(), mujoco_actuator_data_.end(), [&](auto& actuator_interface) {
      if ((actuator_interface.joint_name == joint.name || actuator_interface.joint_name == actuator_name) &&
          actuator_interface.actuator_type != ActuatorType::PASSIVE)
      {
        actuator_interface.position_interface.command_ = joint.position_interface.command_ * scale;
        actuator_interface.velocity_interface.command_ = joint.velocity_interface.command_ * scale;
...
bool MujocoSystemInterface::register_mujoco_actuators()
{
  ...
  for (int i = 0; i < mj_model_->nu; i++)
  {
    ...
    else if (trn_type == mjTRN_TENDON)
    {
      ...
      {
        actuator_data.joint_name = std::string(act_name);
        actuator_data.is_actuator_only = true;
        target_id = -1;
        RCLCPP_INFO(get_logger(),
                    "Registering MuJoCo tendon actuator '%s' as an actuator-only control resource.",
                    act_name);
      }
    }
    else
    ...
    {
      actuator_data.mj_pos_adr = -1;
      actuator_data.mj_vel_adr = -1;
      actuator_data.mj_joint_type = -1;
    }
...
// after humble has an additional argument in the PidROS constructor, and uses a different function to initialize from parameters
  ...
  if (override_mujoco_actuator_positions_)
  {
    ...
    {
      if (actuator_data.is_actuator_only)
      {
        actuator_data.position_interface.state_ = mj_data_->ctrl[actuator_data.mj_actuator_id];
        actuator_data.velocity_interface.state_ = 0.0;
        actuator_data.effort_interface.state_ = mj_data_->actuator_force[actuator_data.mj_actuator_id];
      }
      else
      {
...
void MujocoSystemInterface::register_urdf_joints(const hardware_interface::HardwareInfo& hardware_info)
{
  ...
  for (size_t joint_index = 0; joint_index < hardware_info.joints.size(); joint_index++)
  {
    ...
    for (const auto& command_if : command_interface_names)
    {
      ...
      {
        if (actuator_it->is_actuator_only)
        {
          actuator_it->is_position_control_enabled = true;
          RCLCPP_INFO(get_logger(), "Using direct actuator command interface for resource '%s'", actuator_name.c_str());
          continue;
        }
...
// just disabling for humble because the member variables are different. Could make a different one for humble if desired
      ...
      else if (command_if == hardware_interface::HW_IF_VELOCITY)
      {
        if (actuator_it->is_actuator_only)
        {
          RCLCPP_WARN(get_logger(), "Velocity command interface is not supported for actuator-only resource '%s'",
                      actuator_name.c_str());
          continue;
        }
               ...
               command_if == hardware_interface::HW_IF_FORCE)
      {
        if (actuator_it->is_actuator_only)
        {
          RCLCPP_WARN(get_logger(),
                      "Effort/torque/force command interface is not supported for actuator-only resource '%s'",
                      actuator_name.c_str());
          continue;
        }
...
void MujocoSystemInterface::set_initial_pose()
{
  for (auto& actuator : mujoco_actuator_data_)
  {
    ...
    if (!actuator.is_actuator_only && std::isfinite(actuator.position_interface.state_))
    {
      mj_data_->qpos[actuator.mj_pos_adr] = actuator.position_interface.state_;
    }
    else if (!actuator.is_actuator_only)
    {
      RCLCPP_WARN_EXPRESSION(
...
void MujocoSystemInterface::reset_simulation_state(bool fill_initial_state)
{
  ...
  for (auto& actuator : mujoco_actuator_data_)
  {
    if (actuator.is_actuator_only)
    {
      actuator.position_interface.state_ = mj_data_->ctrl[actuator.mj_actuator_id];
      actuator.velocity_interface.state_ = 0.0;
      actuator.effort_interface.state_ = mj_data_->actuator_force[actuator.mj_actuator_id];
    }
    else
    {
