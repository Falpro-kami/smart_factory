...
class ExecuteSqlUtil:
    ...
    USE_DATABASE_PATTERN = re.compile(r'^\s*USE\s+`?([A-Za-z0-9_]+)`?\s*$', re.IGNORECASE)
    ...
    def execute_multiple_statements(self, query: str) -> List[SQLResult]:
        ...
        for statement in statements:
            try:
                active_database = self._extract_use_database(statement) or active_database
                statement = self._prepare_statement_database_target(statement, active_database)
    ...
    def _extract_use_database(self, statement: str) -> Optional[str]:
        """Return the database selected by a USE statement, if present."""
        match = self.USE_DATABASE_PATTERN.match(self.clean_sql(statement))
        return match.group(1) if match else None

    def _prepare_statement_database_target(self, statement: str, active_database: Optional[str]) -> str:
        """Prevent unqualified CREATE TABLE from falling back to MYSQL_DATABASE."""
        match = self.CREATE_TABLE_PATTERN.match(statement)
        if not match:
            return statement
        ...
        if active_database:
            return f"{match.group(1)}`{active_database}`.`{table_name}`{match.group(3)}"

        if os.getenv("MYSQL_ALLOW_DEFAULT_CREATE_TABLE", "false").strip().lower() in {"1", "true", "yes", "on"}:
            return statement

        raise SQLPermissionError(
            "CREATE TABLE 必须显式指定目标数据库。请使用 `CREATE TABLE db_name.table_name ...`，"
            "或先执行 `USE db_name; CREATE TABLE table_name ...`，避免误建到默认数据库。"
        )
