...
def create_mcp_server(
    ...
    async def get_neo4j_schema(sample_size: Optional[int] = Field(default=config_sample_size, description="The sample size used to infer the graph schema. Larger samples are slower, but more accurate. Smaller samples are faster, but might miss information.")) -> list[ToolResult]:
        """
        ...
        Performance Notes:
            - If `sample_size` is not provided, uses 1000.
        ...
        # `None` means "use server default". Avoid generating invalid Cypher like `sample: None`.
        effective_sample_size = sample_size if sample_size is not None else config_sample_size
        ...
        if effective_sample_size is None:
            ...
            get_schema_query = f"CALL apoc.meta.schema({{sample: {int(effective_sample_size)}}}) YIELD value RETURN value"
    ...
    async def write_neo4j_cypher(
        ...
        If multiple operations are needed, combine them into one continuous Cypher query without semicolons,
...
async def main(
    db_url: str = "bolt://127.0.0.1:7687",
    username: str = "neo4j",
